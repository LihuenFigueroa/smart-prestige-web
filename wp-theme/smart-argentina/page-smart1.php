...
<?php get_template_part('partials/header'); ?>
  ...
  <section id="hero-s1-mobile" style="position:relative; width:100%; aspect-ratio:375/812; overflow:hidden;">
    ...
    <div style="position:absolute; bottom:0; left:0; right:0; z-index:10; padding:0 24px 32px;">
      <h1 class="font-smart-next font-bold text-white" style="font-size:32px; line-height:1.15; margin-bottom:6px;"><?php echo $smart_hero_s1['titulo']; ?></h1>
      <p class="font-smart-sans text-white" style="font-size:14px; line-height:1.4; opacity:0.9; margin-bottom:2px;"><?php echo esc_html($smart_hero_s1['linea_1']); ?></p>
      <p class="font-smart-sans text-white" style="font-size:14px; line-height:1.4; opacity:0.9; margin-bottom:12px;"><?php echo esc_html($smart_hero_s1['linea_2']); ?></p>
      <div>
        <a href="#contacto" class="font-smart-sans" style="display:inline-flex; align-items:center; height:40px; padding:0 24px; background:white; border-radius:9999px; font-size:14px; color:#141413; text-decoration:none; font-weight:700; letter-spacing:-0.025em;"><?php echo esc_html($smart_hero_s1['cta_texto']); ?></a>
      </div>
    </div>
  </section>
  ...
  <section id="hero-s1" class="relative w-full h-screen min-h-[640px] overflow-hidden">
    ...
    <div class="absolute bottom-0 left-0 right-0 z-10" style="padding-left:56px; padding-bottom:56px;">
      <h1 class="font-smart-next font-bold text-white" style="font-size:45px; line-height:1.1; margin-bottom:8px;"><?php echo $smart_hero_s1['titulo']; ?></h1>
      <p class="font-smart-sans text-white" style="font-size:18px; line-height:1.4; opacity:0.9; margin-bottom:2px;"><?php echo esc_html($smart_hero_s1['linea_1']); ?></p>
      <p class="font-smart-sans text-white" style="font-size:18px; line-height:1.4; opacity:0.9; margin-bottom:8px;"><?php echo esc_html($smart_hero_s1['linea_2']); ?></p>
      <p class="font-smart-sans text-white" style="font-size:11px; line-height:1.4; opacity:0.65; margin-bottom:16px;"><?php echo esc_html($smart_hero_s1['disclaimer']); ?></p>
      <div>
        <a href="#contacto" class="font-smart-sans" style="display:inline-flex; align-items:center; height:40px; padding:0 24px; background:white; border-radius:9999px; font-size:14px; color:#141413; text-decoration:none; font-weight:700; letter-spacing:-0.025em;"><?php echo esc_html($smart_hero_s1['cta_texto']); ?></a>
      </div>
    </div>
  ...
  <section id="caracteristicas" class="w-full bg-white px-5 md:px-14 py-10 md:py-16">
    <div class="max-w-[1320px] mx-auto">
      ...
      <div id="comp-s1-img-track" style="overflow-x:hidden;">
        <?php foreach ($smart_versiones_s1 as $v): ?>
        <img src="<?php echo esc_url($v['imagen']); ?>" alt="smart #1 <?php echo esc_attr($v['nombre_version']); ?>" style="width:calc(100vw - 2.5rem); aspect-ratio:3/2; object-fit:cover; object-position:center; flex-shrink:0; display:block;" />
        <?php endforeach; ?>
      </div>
      ...
      <div id="comp-s1-container" style="overflow-x:auto; -webkit-overflow-scrolling:touch; scrollbar-width:none;">
        <div id="comp-s1-grid" style="display:grid; row-gap:12px; align-items:start;">
          ...
          <!-- ROW: imágenes -->
          <?php foreach ($smart_versiones_s1 as $v): ?>
          <img src="<?php echo esc_url($v['imagen']); ?>" alt="smart #1 <?php echo esc_attr($v['nombre_version']); ?>" style="width:100%; aspect-ratio:3/2; object-fit:cover; object-position:center;" />
          <?php endforeach; ?>

          <!-- ROW: modelo / versión -->
          <?php foreach ($smart_versiones_s1 as $v): ?>
          <div class="comp-ver-cell"><p class="comp-model">smart #1</p><p class="comp-version"><?php echo esc_html($v['nombre_version']); ?></p></div>
          <?php endforeach; ?>

          <!-- ROW: divider -->
          <?php foreach ($smart_versiones_s1 as $v): ?><hr class="comp-divider" /><?php endforeach; ?>
  ...
  <section id="elegi-tu-version" class="w-full bg-white" style="overflow:hidden;">
    ...
    <div id="vis-top-bar">
      ...
      <div id="vis-left-content">
        ...
        <div id="vis-btn-row">
          ...
          <a class="vis-manual-link font-smart-sans" href="<?php echo get_template_directory_uri(); ?>/assets/pdf/manual-smart1.pdf" download style="align-items:center; height:40px; padding:0 22px; border-radius:999px; background:#fff; border:1px solid #141413; font-size:13px; font-weight:700; color:#141413; text-decoration:none; white-space:nowrap;">Manual de usuario</a>
        </div>
      </div>
    ...
    <div id="vis-stage" style="overflow:visible;">
      ...
      <div id="vis-car-wrapper" style="position:relative; width:100%;">
        <img id="vis-car-back" src="<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-pure-c1.png" alt="" style="width:100%; max-height:62vh; display:block; object-fit:contain;" />
        <img id="vis-car-front" src="<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-pure-c1.png" alt="smart #1" style="position:absolute; inset:0; width:100%; height:100%; object-fit:contain; transition:opacity 0.4s cubic-bezier(0.25,0,0,1);" />
      </div>
      ...
      <div id="vis-bottom-bar" style="position:relative; z-index:20; display:flex; justify-content:space-between; align-items:flex-end;">
        ...
        <div id="vis-controls-card">
          ...
          <div id="vis-linea-row" style="display:flex; align-items:center; gap:20px; margin-bottom:14px; transition:margin-bottom 0.4s cubic-bezier(0.25,0,0,1);">
            ...
            <button class="vis-linea-btn font-smart-sans" data-linea="Pure"   style="font-size:13px; font-weight:400; color:#141413; background:none; border:none; cursor:pointer; padding:0;">Pure</button>
          ...
          <div id="vis-color-row" style="display:flex; align-items:center; gap:10px; overflow:hidden; max-height:68px; padding:4px 5px 4px 0; transition:max-height 0.4s cubic-bezier(0.25,0,0,1), padding-top 0.4s cubic-bezier(0.25,0,0,1), padding-bottom 0.4s cubic-bezier(0.25,0,0,1);">
            ...
            <div id="vis-color-wrap" style="display:flex; gap:10px; transition:opacity 0.3s ease;">
              <div id="vis-swatches-BRABUS" style="display:none; gap:10px;">
                <button class="vis-color-btn" data-color="1" data-img="<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-c1.png" style="width:28px;height:28px;border-radius:50%;background:linear-gradient(to bottom,#2a2a2a 50%,#bf1e2e 50%);border:none;cursor:pointer;"></button>
              ...
              <div id="vis-swatches-Pro+" style="display:none; gap:10px;">
                ...
                <button class="vis-color-btn" data-color="pp8" data-img="<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-pro-c8.png" style="width:28px;height:28px;border-radius:50%;background:linear-gradient(to bottom,#111111 50%,#0a2c54 50%);border:none;cursor:pointer;"></button>
              </div>
              <div id="vis-swatches-Pure" style="display:flex; gap:10px;">
                <button class="vis-color-btn" data-color="p1" data-img="<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-pure-c1.png" style="width:28px;height:28px;border-radius:50%;background:#111111;border:none;cursor:pointer;box-shadow:0 0 0 2px #fff,0 0 0 3.5px #141413;"></button>
  ...
  <script>
    ...
    var compS1Index = 0;
    var compS1Labels = <?php echo wp_json_encode(array_map(function ($v) { return $v['nombre_version']; }, $smart_versiones_s1)); ?>;
    ...
    (function () {
        ...
        window.switchVis = function(view) {
          ...
          if (view === 'ext') {
            ...
            if (intPanel && window.loadInteriorFor) {
              var linea = window.getVisLinea ? window.getVisLinea() : 'Pure';
      ...
      (function() {
        ...
        var activeSrc   = '<?php echo get_template_directory_uri(); ?>/assets/img/smart1/vis-pure-c1.png';
        var activeLinea = 'Pure';
        var fading      = false;
      ...
      (function() {
        ...
        function updateArrows() {
          ...
          if (btnPrevM) { btnPrevM.style.opacity = idx === 0 ? '0' : '1'; btnPrevM.style.pointerEvents = idx === 0 ? 'none' : ''; }
          if (btnNextM) { btnNextM.style.opacity = idx === imgs.length - 1 ? '0' : '1'; btnNextM.style.pointerEvents = idx === imgs.length - 1 ? 'none' : ''; }
        }
        ...
        [btnPrev, btnNext, btnPrevM, btnNextM].forEach(function(btn) {
          if (!btn) return;
        ...
        if (btnPrevM) btnPrevM.addEventListener('click', function(e) { e.stopPropagation(); goTo(idx - 1); });
        if (btnNextM) btnNextM.addEventListener('click', function(e) { e.stopPropagation(); goTo(idx + 1); });
      })();
...
<script>
  setFddOptions('modelo',<?php echo wp_json_encode(array_map(function ($v) {
    return [$v['slug_form'], 'smart #1 ' . $v['nombre_version']];
  }, $smart_versiones_s1)); ?>);
</script>
</body>
</html>
